...
# v6.0 - 557일 일간 데이터 기반 확정 민감도
# ============================================================
# 핵심:
#   ABS Gap = 실측 Gap + WTI 델타 × (-5.76)  → WTI↑=Gap↓ 현실
#   SM Margin = WTI와 무상관 (R²=0.000) → 독립 유지
#   ET = 양의 상관 +6.13 (일간 557일 기준)
#   ABS Market = +8.12 (양의 상관, 원가보다 느리게 상승)
# ============================================================
...
# ★ 557일 일간 데이터 기반 확정 민감도 (WTI $1/bbl → $/mt 변화)
DEFAULT_SENS = {
    'bz':        16.81,  # R²=0.865 ✓✓
    'et':         6.13,  # R²=0.568 ✓  (일간 기준 양의 상관)
    'sm':        14.75,  # R²=0.822 ✓✓
    'an':         7.87,  # R²=0.569 ✓
    'bd':        20.48,  # R²=0.525 ✓
    'nap':        6.52,  # R²=0.880 ✓✓
    'pr':         4.18,  # R²=0.584 ✓
    'sm_cost':   14.83,  # R²=0.869 ✓✓
    'sm_margin':  -0.08, # R²=0.000 ← 무상관, WTI 독립
    'abs_cost':  13.89,  # R²=0.802 ✓✓
    'abs_mkt':    8.12,  # R²=0.497 ✓  (원가보다 느리게 오름)
    'abs_gap':   -5.76,  # R²=0.541 ✓  WTI↑=Gap↓ ★★★
}
...
def load_gsheet():
    ...
    print(f"  WTI={latest[COL_MAP['wti']]:.2f} | BZ={latest[COL_MAP['bz']]:.0f} | "
          f"ET={latest[COL_MAP['et']]:.0f} | SM={latest[COL_MAP['sm_cn']]:.0f} | "
          f"AN={latest[COL_MAP['an']]:.0f} | BD={latest[COL_MAP['bd']]:.0f} | "
          f"ABS={latest[COL_MAP['abs_mkt']]:.0f}")
    print(f"  ★ ABS Cost={latest['_abs_cost']:.0f} | ABS Gap={latest['_abs_gap']:.0f} | "
          f"SM Margin={latest['_sm_margin']:.0f}")
    return latest, df, hist8

# ──────────────────────────────────────────────
# 3. 자동 회귀계수 (구글시트 누적 데이터 보정용)
#    기본값은 557일 확정 민감도, 구글시트 R²>0.5 시 자동 갱신
# ──────────────────────────────────────────────
def calc_regression(df):
    ...
    sens = dict(DEFAULT_SENS)  # 기본값으로 시작
    r2   = {k: None for k in sens}
    n    = 0

    print("\n[ 자동 회귀계수 (기본값: 557일 확정, R²>0.5 시 자동 갱신) ]")
    R2_THRESHOLD = 0.5  # 더 엄격한 기준 적용

    for key, col in {**raw_targets, **derived_targets}.items():
        valid = df[[wti_col, col]].dropna()
        n_pts = len(valid)
        if n_pts < 6:
            print(f"  {key:14s}: 데이터 부족({n_pts}주) → 기본값 {DEFAULT_SENS[key]:+.2f}")
            continue
        ...
        if r2v >= R2_THRESHOLD:
            ...
            flag = f'→ 기본값 {DEFAULT_SENS[key]:+.2f} 유지'
...
def calc_costs(latest, wti_rt, sens):
    ...
    d = wti_rt - wti_gs  # WTI 델타

    return {
        'WTI_RT':         round(wti_rt, 2),
        'WTI_GS':         round(wti_gs, 2),
        'WTI_Delta':      round(d, 2),
        'NAP':            round(nap_act + d * sens['nap'], 1),
        'BZ':             round(bz_act  + d * sens['bz'],  1),   'BZ_Actual':       round(bz_act, 1),
        'ET':             round(et_act  + d * sens['et'],  1),   'ET_Actual':       round(et_act, 1),
        'SM_Market':      round(sm_act  + d * sens['sm'],  1),   'SM_Actual':       round(sm_act, 1),
        'BD':             round(bd_act  + d * sens['bd'],  1),   'BD_Actual':       round(bd_act, 1),
        'AN':             round(an_act  + d * sens['an'],  1),   'AN_Actual':       round(an_act, 1),
        # SM Cost / Margin
        'SM_Cost':        round(sm_cost_act   + d * sens['sm_cost'],   1), 'SM_Cost_Actual':   round(sm_cost_act, 1),
        'SM_Margin':      round(sm_margin_act + d * sens['sm_margin'], 1), 'SM_Margin_Actual': round(sm_margin_act, 1),
        # ABS - Gap 직접 회귀 ★
        'ABS_Market':     round(abs_mkt_act   + d * sens['abs_mkt'],  1), 'ABS_Mkt_Actual':  round(abs_mkt_act, 1),
        'ABS_Cost':       round(abs_cost_act  + d * sens['abs_cost'], 1), 'ABS_Cost_Actual': round(abs_cost_act, 1),
        'ABS_Gap':        round(abs_gap_act   + d * sens['abs_gap'],  1), 'ABS_Gap_Actual':  round(abs_gap_act, 1),
    }
...
def calc_scenario(latest, sens, wti_target, risk_premium):
    ...
    r['ABS_Gap']    += risk_premium  # Gap에도 리스크 프리미엄 직접 가산
    r['Risk']        = risk_premium
    return r
...
def generate_report(current, hist8, latest, sens, r2, n_reg, wti_source):
    ...
    fig.suptitle(
        f'IRAN CONFLICT RISK DASHBOARD  |  WTI ${current["WTI_RT"]:.2f}/bbl  |  '
        f'실측앵커: {gs_date}  |  557일 확정민감도 v6.0  |  '
        f'{datetime.datetime.now().strftime("%Y-%m-%d %H:%M UTC")}',
        fontsize=11, fontweight='bold', color='#fbbf24', y=0.98
    )
    ...
    def c(lst): return [v if (v and not (isinstance(v,float) and np.isnan(v))) else float('nan') for v in lst]

    sp_kw = {'facecolor':'#1e293b'}
    def border(ax):
        for sp in ax.spines.values(): sp.set_edgecolor('#334155')

    # ── 차트1: ABS Gap 8주 바 차트 ★ 핵심
    ax1 = fig.add_subplot(2,3,1); ax1.set_facecolor('#1e293b')
    ...
    for i,g in enumerate(abs_gap_h):
        if not np.isnan(g):
            ax1.text(i, g+(8 if g>=0 else -22), f'${g:.0f}',
    ...
    ax1.set_title(f'ABS Gap 8주 실측 | 현재 실측 ${current["ABS_Gap_Actual"]:+.0f} → 보정 ${current["ABS_Gap"]:+.0f}/t',
    ...
    # ── 차트2: ABS Market vs Cost
    ax2 = fig.add_subplot(2,3,2); ax2.set_facecolor('#1e293b')
    ...
    # ── 차트3: SM Margin 8주 바
    ax3 = fig.add_subplot(2,3,3); ax3.set_facecolor('#1e293b')
    sm_cols = ['#10b981' if m>=0 else '#ef4444' for m in sm_margin_h]
    ax3.bar(list(x), sm_margin_h, color=sm_cols, alpha=0.85, edgecolor='white', linewidth=0.5)
    for i,m in enumerate(sm_margin_h):
        if not np.isnan(m):
            ax3.text(i, m+(5 if m>=0 else -20), f'${m:.0f}',
                     ha='center', fontsize=8, color='white', fontweight='bold')
    ax3.axhline(y=0, color='white', linewidth=1, alpha=0.4)
    smc = '#10b981' if current['SM_Margin']>=0 else '#ef4444'
    ax3.set_title(f'SM Margin 8주 (WTI 무상관 R²≈0) | 현재 ${current["SM_Margin_Actual"]:+.0f}/t',
                  color=smc, fontweight='bold', fontsize=9)
    ...
    # ── 차트4: SM Market vs Cost
    ax4 = fig.add_subplot(2,3,4); ax4.set_facecolor('#1e293b')
    ax4.fill_between(x, sm_cost_h, sm_h, alpha=0.15,
    ...
    ax4.set_title('SM Market vs Cost (8W)', color='#3b82f6', fontweight='bold')
    ...
    # ── 차트5: 원료가 트렌드 + WTI
    ax5 = fig.add_subplot(2,3,5); ax5.set_facecolor('#1e293b')
    ax5r = ax5.twinx()
    ax5r.plot(x, wti_h, color='#fbbf24', linewidth=2, marker='^', markersize=5, linestyle='--', label='WTI(R)')
    ax5r.set_ylabel('WTI $/bbl', color='#fbbf24', fontsize=9)
    ax5r.tick_params(axis='y', colors='#fbbf24')
    ax5r.set_ylim(40, 110)
    ax5.plot(x, bz_h,    color='#a855f7', linewidth=2, marker='o', markersize=5, label='BZ')
    ax5.plot(x, et_h,    color='#10b981', linewidth=2, marker='s', markersize=4, label='ET')
    ax5.plot(x, c(an_h), color='#f59e0b', linewidth=2, marker='D', markersize=4, label='AN')
    ax5.plot(x, c(bd_h), color='#f97316', linewidth=2, marker='v', markersize=4, label='BD')
    ...
    # ── 차트6: 이란 시나리오 ABS Gap
    ax6 = fig.add_subplot(2,3,6); ax6.set_facecolor('#1e293b')
    sc_labels, sc_gaps, sc_costs_s, sc_mkts_s, sc_clrs = [], [], [], [], []
    for s in SCENARIOS:
        r = calc_scenario(latest, sens, s['wti'], s['risk'])
        sc_labels.append(s['label'])
        sc_gaps.append(r['ABS_Gap'])
        sc_costs_s.append(r['ABS_Cost'])
        sc_mkts_s.append(r['ABS_Market'])
        sc_clrs.append(s['color'])
    ...
    for i,(g,c_,m_) in enumerate(zip(sc_gaps, sc_costs_s, sc_mkts_s)):
    ...
    ax6.set_title('Iran Risk Scenario: ABS Gap (Gap직접회귀)', color='#fbbf24', fontweight='bold')
    ax6.set_xticks(xp); ax6.set_xticklabels(sc_labels, fontsize=8, color='white')
    ...
    ax6.set_ylim(0, max(all_vals)*1.25)
    border(ax6)

    fig.text(0.5, 0.01,
             f'LAM Advanced Procurement  |  ABS Gap sens={sens["abs_gap"]:+.2f}/bbl (557일)  '
             f'|  SM Margin R²≈0 (WTI 무상관)  |  {wti_source}',
...
def save_csv(current, sens, r2, n_reg, wti_source, gs_date):
    ...
    row = {
        'UpdateTime':       now,      'WTI_Source':       wti_source,
        'GSheet_Date':      gs_date,  'Reg_N':            n_reg,
...
if __name__ == "__main__":
    print("="*65)
    print("Iran Risk × ABS/SM 원가 시뮬레이션 v6.0")
    print("557일 확정 민감도 | ABS Gap 직접회귀 | SM Margin WTI 무상관")
    print("="*65)
    ...
    print(f"  BZ           : ${current['BZ_Actual']:.0f} → ${current['BZ']:.0f}  (sens {sens['bz']:+.2f})")
    print(f"  ET           : ${current['ET_Actual']:.0f} → ${current['ET']:.0f}  (sens {sens['et']:+.2f})")
    print(f"  SM           : ${current['SM_Actual']:.0f} → ${current['SM_Market']:.0f}  (sens {sens['sm']:+.2f})")
    print(f"  AN           : ${current['AN_Actual']:.0f} → ${current['AN']:.0f}  (sens {sens['an']:+.2f})")
    print(f"  BD           : ${current['BD_Actual']:.0f} → ${current['BD']:.0f}  (sens {sens['bd']:+.2f})")
    print(f"  ─────────────────────────────────────────────────")
    print(f"  SM Cost      : ${current['SM_Cost_Actual']:.0f} → ${current['SM_Cost']:.0f}/t")
    print(f"  SM Margin    : ${current['SM_Margin_Actual']:+.0f} → ${current['SM_Margin']:+.0f}/t  [WTI 무상관]")
    ...
    for s in SCENARIOS:
        ...
        print(f"  {s['label'].replace(chr(10),' '):22s} | WTI ${s['wti']:5.0f} | "
              f"Risk +${s['risk']:3.0f} | Gap ${r['ABS_Gap']:+.0f}/t {flag}")
